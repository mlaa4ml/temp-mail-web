...
- ✍ Создание ящика с собственным логином (проверка занятости — по 403 при попытке открыть, пока воркер на сервере не обновлён отдельным эндпоинтом `/mailbox/availability`)
...
| `BASE_PATH` | Базовый путь для статики (например, `/temp-mail-web/` для GitHub Pages) | `/` |
...
| GET | `/mailbox/availability` | Проверка доступности собственного логина (`?login=&domain=`) — временно не используется, ожидает деплоя воркера на сервере |

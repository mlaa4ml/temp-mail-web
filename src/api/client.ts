...
export const api = {
	 ...
	 *
	 * @deprecated Временно не используется: эндпоинт `/mailbox/availability` ещё
	 * не задеплоен на сервере. После деплоя вернуть вызов из `MailboxSetup` и
	 * снять атрибут @deprecated. Метод сохранён, чтобы потом включить проверку
	 * одним изменением.
	 */
